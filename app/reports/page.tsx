...
export default function ReportsPage() {
  ...
  // ตั้งค่า default range ตาม period
  useEffect(() => {
  ...
  const fetchReport = async (loc: string, p: Period, start: string, end: string): Promise<ReportResp> => {
    const q = new URLSearchParams({ location: loc, period: p, start, end }).toString();
  ...
  const load = async () => {
    ...
    try {
      if (location !== 'ALL') {
        const rep = await fetchReport(location, period, rangeStart, rangeEnd);
        setSingleReport(rep);
      } else {
        if (allLocations.length === 0) await loadLocations();
        ...
        const results = await Promise.all(
          locs.map(l => fetchReport(l.id, period, rangeStart, rangeEnd))
        );
  ...
  // รวม grand totals เมื่อเป็น ALL (ทุก period)
  ...
  const isSimpleView = period === 'weekly' || period === 'monthly';
      ...
      {!location ? (
        ...
        singleReport ? (
          <section className="space-y-4">
            ...
            {isSimpleView ? (
              // ===== Weekly / Monthly: แสดงเรียบง่าย 3 คอลัมน์ =====
              <div className="bg-white rounded-xl border p-4">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-left border-b">
                        ...
                        <th className="py-2 pr-3">ยอดเงินที่ได้ (บาท)</th>
                        <th className="py-2 pr-3">ยอดจำนวนชิ้น (ชิ้น)</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr className="border-b last:border-0">
                        <td className="py-2 pr-3">{singleReport.location}</td>
                        <td className="py-2 pr-3">{fmt(singleReport.grand.totalAmount)}</td>
                        <td className="py-2 pr-3">{singleReport.grand.totalQty}</td>
                      </tr>
        ...
        // ---------- โหมด ALL ----------
        <>
          {/* Weekly/Monthly: ตารางรวมสาขาแบบ 3 คอลัมน์ */}
          {isSimpleView ? (
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {multiReports.map((rep) => (
                        <tr key={rep.location} className="border-b last:border-0">
                          <td className="py-2 pr-3">{rep.location}</td>
                          <td className="py-2 pr-3">{fmt(rep.grand.totalAmount)}</td>
                          <td className="py-2 pr-3">{rep.grand.totalQty}</td>
                        </tr>
                      ))}
                      {allGrand && (
                        <tr className="font-semibold">
                          <td className="py-2 pr-3">รวมทั้งหมด</td>
                          <td className="py-2 pr-3">{fmt(allGrand.totalAmount)}</td>
                          <td className="py-2 pr-3">{allGrand.totalQty}</td>
                        </tr>
                      )}
                      {multiReports.length === 0 && (
                        <tr><td colSpan={3} className="py-3 text-gray-500">No data</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
            ...
            // Daily (ALL): โชว์ summary รวม + ตารางแยกสาขาตามของเดิม
            <>
              {allGrand && (
                <div className="bg-white rounded-xl border p-4 mb-4">
                  ...
                  <div className="flex flex-wrap gap-3 text-sm">
                    ...
                    <div className="bg-white rounded-xl border p-3">Total Amount: <b>{fmt(allGrand.totalAmount)}</b> THB</div>
                    <div className="bg-white rounded-xl border p-3">Freebies Amount: <b>{fmt(allGrand.freebiesAmount)}</b> THB</div>
                  </div>
                </div>
              )}
                  ...
                  {multiReports.map((rep) => (
                    <section key={rep.location} className="bg-white rounded-xl border p-4">
                      ...
                      <div className="flex flex-wrap gap-3 text-sm mb-3">
                        ...
                        <div className="bg-white rounded-xl border p-3">Total Amount: <b>{fmt(rep.grand.totalAmount)}</b> THB</div>
                        <div className="bg-white rounded-xl border p-3">Freebies Amount: <b>{fmt(rep.grand.freebiesAmount)}</b> THB</div>
                      </div>
