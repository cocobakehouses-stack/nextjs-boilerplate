...
export default function POSPage() {
  ...
  const [freebiePick, setFreebiePick] = useState<number | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setProducts(list);
        // ตั้งค่า default ของ freebiePick เมื่อมีสินค้า
        if (list.length > 0) setFreebiePick(list[0].id);
      } catch (e: any) {
        setProdErr(e?.message || 'Load products failed');
      } finally {
  ...
  const grouped = useMemo(() => {
    ...
    for (const p of products) {
      ...
      // ช่วง 110–125 จะไม่เข้าหมวด (ถ้าต้องการให้เข้าหมวด Soft ให้ else soft.push(p);)
    }
    ...
    <main className="min-h-screen p-4 sm:p-6 lg:p-8 bg-[#fffff0]">
      ...
      <div className="mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-gray-700">Location: <b>{location ?? '— เลือกก่อนใช้งาน —'}</b></span>
          <button onClick={() => { localStorage.removeItem('pos_location'); setLocation(null); }} className="px-3 py-1 rounded-lg border hover:bg-white">
            เปลี่ยนสถานที่
          </button>
          {location && (
            <a className="px-3 py-1 rounded-lg border hover:bg-white" target="_blank" rel="noreferrer"
               href={`/history?location=${encodeURIComponent(location)}&date=${encodeURIComponent(dateStr)}`} title="Show history (open in new tab)">
              Show history
            </a>
          )}
        </div>
        ...
        <>
          ...
          <div className="mb-4 flex gap-2 text-sm">
            {(['cart', 'summary', 'confirm', 'success'] as Step[]).map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                <div className={classNames('w-7 h-7 rounded-full flex items-center justify-center',
          ...
          {step === 'cart' && (
            <>
              {/* สถานะโหลดเมนู */}
              {loadingProducts ? (
                <div className="mb-4 text-gray-600">Loading menu…</div>
              ) : prodErr ? (
                <div className="mb-4 text-red-600">โหลดเมนูไม่สำเร็จ: {prodErr}</div>
              ) : null}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                      {items.map((p) => (
                        <div key={p.id} className="border rounded-xl p-4 flex flex-col items-center bg-white">
                          ...
                          <button
                            onClick={() => addToCart(p)}
                            className={classNames('mt-auto w-full px-4 py-2 rounded text-[#fffff0]',
              ...
              <div className="fixed bottom-0 left-0 right-0 bg-[#fffff0]/95 backdrop-blur border-t">
                <div className="mx-auto max-w-6xl p-3 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-sm">รวมจำนวน: <b>{totalQty}</b> | ยอดรวม: <b>{subtotal} THB</b></div>
                  </div>
                  <button onClick={goSummary} className="px-4 py-2 rounded-lg bg-[#ac0000] text-[#fffff0] hover:opacity-90 disabled:opacity-40"
                          ...
                          {cart.map((i) => (
                            <div key={i.id} className="flex justify-between items-center border-b pb-2">
                              <div>
                                ...
                                <div className="text-sm text-gray-600">{i.price} THB × {i.quantity}</div>
                              </div>
                              <div className="flex items-center gap-2">
          ...
          {step === 'confirm' && (
            <div className="bg-white rounded-xl p-6 border max-w-xl">
              ...
              <div className="mt-6 flex gap-2">
                <button onClick={() => setStep('summary')} className="px-4 py-2 rounded-lg border hover:bg-gray-50" disabled={isSubmitting}>กลับไปแก้</button>
          ...
          {step === 'success' && lastSaved && (
            <div className="bg-white rounded-xl p-6 border max-w-xl">
              ...
              <div className="mt-6">
                <button onClick={resetForNewBill} className="px-4 py-2 rounded-lg bg-[#ac0000] text-[#fffff0] hover:opacity-90">เริ่มบิลใหม่</button>
              </div>
            </div>
          )}
        </>
